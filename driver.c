...
// START of assignment code section 
	...
	if (err != CL_SUCCESS)
	{
		fprintf(stderr, "Error: Failed to write to input array %d to device memory!\n", err);
		exit(EXIT_FAILURE);
	}
	...
	// Wait for the command command_queue to get serviced before reading back results. This is the device sending an interrupt to the host    
	...
	if (err != CL_SUCCESS) {
		fprintf(stderr, "Error: Failed to release memory object: %d!/n", err);
		exit(EXIT_FAILURE);
	}
